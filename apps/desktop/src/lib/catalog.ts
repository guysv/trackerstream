...
export const listPlaylists = (): Promise<PlaylistSummary[]> =>
  api<{ playlists: PlaylistSummary[] }>("/playlists").then((r) => r.playlists);

export const getPlaylist = (id: number): Promise<Playlist> => api<Playlist>(`/playlist/${id}`);

export const createPlaylist = (name: string, isPublic: boolean, items: number[]): Promise<Playlist> =>
  send<Playlist>("/playlists", "POST", { name, isPublic, items });

export const updatePlaylist = (
  id: number,
  patch: { name?: string; isPublic?: boolean; items?: number[] },
): Promise<Playlist> => send<Playlist>(`/playlist/${id}`, "PUT", patch);

export const deletePlaylist = (id: number): Promise<void> =>
  send(`/playlist/${id}`, "DELETE").then(() => undefined);
