...
**Scope**
- **P2P stack (decided + revised in implementation): master = `kubo` (Node over kubo RPC); client = an in-process `rust-ipfs` node embedded in the Tauri Rust backend — NO sidecar / second process.** (The original "bundled kubo sidecar" pick was revised: the client compiles rust-ipfs directly into the binary; it interops with the kubo master over the same Bitswap + CIDv1/sha2-256 block scheme. Helia-in-webview was also dropped.) **Block/codec format (settled): raw-codec chunk leaves + `dag-cbor` manifest/pcm-roots** (custom IPLD, not UnixFS) — see [`lab/CID.md`](lab/CID.md).
