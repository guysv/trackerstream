<script lang="ts">
  ...
  import { peers, selection, selectPeer } from "../peers.svelte";
  import { fmtBytes } from "../format";
  import PeerDetail from "./PeerDetail.svelte";

  // There is no log FOLDER to open in a browser — the shell logs to the console. Hide the button
  // rather than surface a dead control.
  const hasLogsDir = can("logsDir");

  function openLogs(): void {
    void client().platform.openLogsDir?.().catch(() => {});
  }
